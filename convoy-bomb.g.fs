...
FORTH DEFINITIONS HEX ( Z80 MACHINE CODE)
...
: INST CLS CR CR 6 SPACES ." C O N V O Y   B O M B" CR CR
  ." DITT UPPDRAG AR ATT SANKA ALLA" CR
  ." FARTYGEN I EN KONVOJ. KONVOJEN" CR
  ." BESTAR AV 5 HANDELSFARTYG." CR
  ." DU FAR POANG F0R VARJE SANKT" CR
  ." SKEPP OCH POANGAVDRAG F0R VARJE" CR
  ." 0VERFLYGNING." CR
  ." F0R ATT FALLA EN BOMB, TRYCK NER" CR
  ." <B> TILLS DU SER BOMBEN FALLA." CR
  ." DU KAN INTE FALLA NASTA BOMB" CR
  ." F0RREN DEN F0RRA HAR LANDAT." CR
  ." DU KAN EJ REGLERA H0JDEN EFTER-" CR
  ." SOM DU EJ AR PILOT UTAN BOMB-" CR
  ." FALLARE." CR
  ." F0R ATT B0RJA, SKRIV <N/L>." ;
: CONVOY-BOMB HIDE-CUR BEGIN INST VISA B HOME CR CR CR CR
  ." VILL DU SPELA EN GANG TILL?(J/N)"
  0 BEGIN DROP KEY DUP DUP [CHAR] n = SWAP [CHAR] j = OR UNTIL
  [CHAR] n = UNTIL CLS
  ." ADJ0. HOPPAS NI HADE TREVLIGT." CR SHOW-CUR ;
( ;S )
